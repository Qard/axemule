var xml = require('./xml')
  , util = require('util')

/**
 * Construct HTML document
 * 
 * @param string
 *    String prefix, typically doctype
 */
function html (text) {
  if ( ! (this instanceof html)) {
    return new html(text)
  }
  xml.call(this, (typeof text !== 'undefined') ? text : '<!DOCTYPE html>')
}
util.inherits(html, xml)

/**
 * Define all legal HTML5 tags
 */
var types = [
  'a', 'abbr', 'acronym', 'address', 'applet', 'area', 'article', 'aside',
  'audio', 'b', 'base', 'basefont', 'bdi', 'bdo', 'big', 'blockquote',
  'body', 'br', 'button', 'canvas', 'caption', 'center', 'cite', 'code',
  'col', 'colgroup', 'command', 'datalist', 'dd', 'del', 'details', 'dfn',
  'dir', 'div', 'dl', 'dt', 'em', 'embed', 'fieldset', 'figcaption', 'figure',
  'font', 'footer', 'form', 'frame', 'frameset', 'h1', 'h2', 'h3', 'h4', 'h5',
  'h6', 'head', 'header', 'hgroup', 'hr', 'html', 'i', 'iframe', 'img',
  'input', 'ins', 'keygen', 'kbd', 'label', 'legend', 'li', 'link', 'map',
  'mark', 'menu', 'meta', 'meter', 'nav', 'noframes', 'noscript', 'object',
  'ol', 'optgroup', 'option', 'output', 'p', 'param', 'pre', 'progress', 'q',
  'rp', 'rt', 'ruby', 's', 'samp', 'script', 'section', 'select', 'small',
  'source', 'span', 'strike', 'strong', 'style', 'sub', 'summary', 'sup',
  'table', 'tbody', 'td', 'textarea', 'tfoot', 'th', 'thead', 'time', 'title',
  'tr', 'track', 'tt', 'u', 'ul', 'var', 'video', 'wbr'
]

/**
 * Create aliases to add() for every available flag
 */
types.forEach(function (type) {
  html.prototype[type] = function (attr, cb) {
    this.add(type, attr, cb)
  }
})

module.exports = html
module.exports.types = types